...
const Dashboard = {
  ...
  renderCompeticaoSection() {
    ...
    // NÃO carrega dados salvos - campos sempre começam vazios
    const formGrid = DOM.create("div", { className: "form-grid" });
    formGrid.innerHTML = `
      <div class="full">
        <label>Nome da Competição:</label>
        <input type="text" id="compNome" value="" placeholder="Ex: TRIF 2025 - 6ª Edição">
      </div>
      <div class="full">
        <label>Local:</label>
        <input type="text" id="compLocal" value="" placeholder="Ex: IFSP - Campus São Paulo">
      </div>
      <div class="full">
        <label>Data:</label>
        <input type="date" id="compData" value="">
      </div>
      <div class="full">
        <label>Descrição:</label>
        <textarea id="compDesc" rows="3" placeholder="Descrição do evento..."></textarea>
      </div>
  ...
  limparCompeticao() {
    ...
    // Não mostra notificação aqui para evitar duplicação quando chamado por salvarCompeticao()
  },
  ...
  salvarCompeticao() {
    try {
      ...
      // Limpa os campos ANTES de mostrar a notificação
      this.limparCompeticao();
  ...
  limparRobo() {
    ...
    // Não mostra notificação aqui para evitar duplicação quando chamado por salvarRobo()
  },
  ...
  async salvarRobo() {
    try {
      ...
      toast.success(
        roboExistente
          ? `Robô "${nome}" atualizado com sucesso!`
          : `Robô "${nome}" cadastrado com sucesso!`
      );
